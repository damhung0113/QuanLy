...
$ds_bo_mon = get_ds_bo_mon($offset, $total_records_per_page);
$content = '';
while ($row = mysqli_fetch_row($ds_bo_mon)) {
  $content .= '<tr><td>' . $row[0] .
      '</td><td>' . $row[1] .
      '</td><td>' . mysqli_fetch_object(get_ten_khoa($row[2]))->Ten_khoa .
    ...
    <div class="col-md-10 row container-cus pl-5">
      <div class="col-md-12">
        <div class="title_chart" style="width: 25%">
          <span class="text-uppercase">Danh sách bộ môn</span>
          <span class="arrow_chart"></span>
        </div>
        <div class="card">
          <div class="card-header">
            <a href="create_bo_mon.php" class="btn btn-outline-primary float-right" role="button"
               style="border-color: #007bff;">Tạo mới bộ môn</a>
          </div>
